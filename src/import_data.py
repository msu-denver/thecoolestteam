    ...
    with app.app_context():
        with open(csv_filepath, encoding='utf-8') as csvfile:
            ...
            for row in reader:
                ...
                try:
                    numVotes = int(float(row.get('numVotes', 0))) if row.get('numVotes') else None
                except ValueError:
                    numVotes = None
                try:
                    releaseYear = int(float(row.get('releaseYear', 0))) if row.get('releaseYear') else None
                except ValueError:
                    releaseYear = None
                ...
                if type_ == 'movie':
                    ...
                    if existing_movie:
                        print(f"Row {total_rows}: Movie with ID '{entry_id}' already exists. Skipping.")
                        skipped_rows += 1
                        continue
                    ...
                    if existing_tvshow:
                        print(f"Row {total_rows}: TV show with ID '{entry_id}' already exists. Skipping.")
                        skipped_rows += 1
                        continue
                    ...
                    tvshow = TVShow(
                        ...
                        release_date=row.get('release_date', '').strip() or datetime.now(),
...
if __name__ == '__main__':
    ...
    csv_path = os.path.join(os.path.dirname(__file__), 'data', 'imdb_full.csv')
